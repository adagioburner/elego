...
for (const limit of PRUNING_LIMITS) {
    configs.push({
        expansionStrategy: 'Pruned',
        simulationMode: 'Hybrid',
        proximityScoreMax: parsedProximityScoreMax,
        proximityScoreMin: parsedProximityScoreMin,
        pruningLimit: limit
    });
}
...
function configToString(config: AiConfig): string {
    return `Pruned(${config.pruningLimit})-Hybrid-${config.proximityScoreMax}-${config.proximityScoreMin}`;
}
...
async function playSingleGame(configBlack: AiConfig, configWhite: AiConfig): Promise<Player> {
    ...
    playerBlack.setPruningLimit(configBlack.pruningLimit);
    ...
    playerWhite.setPruningLimit(configWhite.pruningLimit);
...
if (!isMainThread) {
    ...
    const OUT_FILE = path.join(__dirname, '..', 'benchmark_pruning_results.csv');

    async function main() {
        console.log('Starting AI Pruning Benchmarks...');
